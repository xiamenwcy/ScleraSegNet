...
parser.add_argument('--model', type=str, default='Sclera_ubiris.caffemodel') 
...
test_dir = '/data3/caiyong.wang/data/sclera/uburis/test/image/' # test images directory
save_mask_dir = join('/data3/caiyong.wang/ScleraSegNet/CBAM/test/uburis/', splitext(split(args.model)[1])[0],'mask/') # directory to save results
if not os.path.exists(save_mask_dir):
    os.makedirs(save_mask_dir) 
    
    
imgs = [i for i in os.listdir(test_dir) if '.tiff' in i]
nimgs = len(imgs)
...
for i in range(nimgs):
  ...
  mask_result = crop_image(net.blobs['sigmoid_fuse_mask_1'].data[0][0,:,:], pads)
  
  scipy.misc.imsave(join(save_mask_dir, fn+'.png'),mask_result)
